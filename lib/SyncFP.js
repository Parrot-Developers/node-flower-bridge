...
function SyncFP(flowerPowerUuid, options, user, api) {
	TaskFP.call(this, flowerPowerUuid);
	this.actions = [
		{'live': options.uploud || false},
...
SyncFP.prototype.start = function(options) {
	async.series([
		this.findAndConnect,
	], function(err, res) {
		console.log('end');
...
SyncFP.prototype.syncSamples = function(callback) {
	...
	self.getSamples(cloudIndex, function(err, dataBLE) {
		...
		self.api.sendSamples(param, function(error) {
			if (!error) {
				self.state = 'Updated';
