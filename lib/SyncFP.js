...
SyncFP.prototype.syncStatus = function(callback) {
	...
	self.getStatusWatering(function(err, watering) {
		...
		param["user_config_version"] = self.user.user_config_version;
		console.log(2);
		update_status['location_identifier'] = self.user.locations.sensor[self.FP.name].location_identifier;
		update_status['status_creation_datetime_utc'] = self.user.locations.sensor[self.FP.name].status_creation_datetime_utc;
