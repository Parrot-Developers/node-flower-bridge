...
FlowerBridge.prototype.for = function(sensors, actions) {
	this.queue.push({
		sensors: (sensors === true) ? 'all' : sensors,
		actions: {
			live: actions.live || false,
			upload: actions.upload || false,
			update: actions.update || false
		}
	});

	console.log(this.queue[0].actions);
};

FlowerBridge.prototype.initBridge2 = function() {
	var self = this;

	self.queue = async.queue(function(task, callbackNext)
	}, 1);
};
		...
		if (err) self.error('Error in getInformationsCloud');
			...
			for (var identifier in user.sensors) {
				if (typeof options.filter != 'undefined') {
					if (options.filter(user.sensors[identifier])) {
						sensors.push(helpers.uuidCloudToPeripheral(identifier));
					}
				}
				else {
					sensors.push(helpers.uuidCloudToPeripheral(identifier));
			...
			for (var sensor of sensors) {
				self[action](sensor, options, user);
			}
		}
	});
};
