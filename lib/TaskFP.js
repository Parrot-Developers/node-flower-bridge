...
TaskFP.prototype.readDataBLE = function(keys) {
	return new Promise((resolve, reject) => {
		...
		for (const i in keys) {
			array[keys[i]] = makeFn(this.charac[keys[i]]);
		}
		async.parallel(array, (err, results) => {
			if (err) reject(err);
...
TaskFP.prototype.search = function(callback) {
	...
	const discover = (device) => {
		if (device.uuid != this.uuid) return this.destroy(device);
...
TaskFP.prototype.connect = function(callback) {
	...
	setTimeout(() => {
		if (this.process[0] == 'Connection') {
			...
			this.destroy(this.FP);
			throw (this.FP.uuid + ': Connection failed');
		}
